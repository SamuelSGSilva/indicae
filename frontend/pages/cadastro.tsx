  ...
  return (
    <div style={{ maxWidth: '400px', margin: '60px auto', padding: '30px', boxShadow: '0 4px 15px rgba(0,0,0,0.1)', borderRadius: '12px', textAlign: 'center', backgroundColor: '#fff' }}>
      ...
      <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <input style={inputStyle} type="text" placeholder="Nome Completo" onChange={e => setFormData({...formData, name: e.target.value})} required />
        <input style={inputStyle} type="email" placeholder="Seu melhor e-mail" onChange={e => setFormData({...formData, email: e.target.value})} required />
        <input style={inputStyle} type="text" placeholder="GitHub User (Opcional)" onChange={e => setFormData({...formData, github: e.target.value})} />
        <input style={inputStyle} type="password" placeholder="Senha (8+ chars, A-Z, 0-9)" onChange={e => setFormData({...formData, password: e.target.value})} required />
        <input style={inputStyle} type="password" placeholder="Confirme sua Senha" onChange={e => setFormData({...formData, confirmPassword: e.target.value})} required />
